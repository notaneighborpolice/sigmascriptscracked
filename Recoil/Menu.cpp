...
#include "../Recoil/Imgui/imgui.h"
#include "../Recoil/Imgui/imgui_impl_dx9.h"
#include "../Recoil/Imgui/imgui_impl_win32.h"
#include "Settings.h"
#include "Utils.h"
#include "Recoil.h"
#include "Globals.h"
...
static inline void SetupImGuiStyle()
{
    ...
    const ImVec4 panelHoverColor = ColorFromBytes(20, 20, 20);
    const ImVec4 panelActiveColor = ColorFromBytes(201, 38, 119);
    ...
    style.ChildRounding = 5.1f;
    style.FrameRounding = 5.1f;
    style.GrabRounding = 5.1f;
    style.PopupRounding = 5.1f;
    style.ScrollbarRounding = 5.1f;
    style.TabRounding = 5.1f;
}
...
int menu()
{
    ...
    while (msg.message != WM_QUIT)
    {
        ...
        if (globals::menu_active)
        {
            try
            {
                ImGui::Begin(skCrypt("SigmaScripts.xyz"), p_open, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse);  // Create our main window and append into it.
                ...
                ImGui::Hotkey(skCrypt("Next Weapon    "), &g_settings.weapon_cycler, ImVec2{ 220, 20 });
                ImGui::Hotkey(skCrypt("Next Scope     "), &g_settings.scope_cycler, ImVec2{ 220, 20 });
                ImGui::Hotkey(skCrypt("Next Barrel    "), &g_settings.barrel_cycler, ImVec2{ 220, 20 });
...
void find_rust()
{
    while (true)
    {
        try
        {
            globals::rust = FindWindow(0, skCrypt(L"Untitled - Notepad"));
